...
scheduler.templates.tooltip_text = function(start, end, event) {
	var name = event.text
	if (name.indexOf(' - forever') >= 0) name = name.split(' - forever')[0]
	else if (name.indexOf(' - occurrence') >= 0) name = name.split(' - occurrence')[0]
	...
	if (event.schedule) html += "<b>Schedule: </b><pre>" + event.schedule + "</pre><br/>"
	if (event.description) html += "<b>Description: </b>" + event.description + "<br/>"
